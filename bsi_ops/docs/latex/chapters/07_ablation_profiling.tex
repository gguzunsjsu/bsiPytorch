...
This ablation matters because it establishes that the problem cannot be framed purely as a global bitwidth problem. The same bit policy has different consequences in different layer groups. That behavior is consistent with the broader low-bit inference literature, where compression quality depends strongly on outlier structure, salient channels, and the placement of quantization error within the network \cite{gupta2015limitedprecision,llmint8,awq,smoothquant}.
