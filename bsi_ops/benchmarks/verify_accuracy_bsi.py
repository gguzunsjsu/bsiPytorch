...
def _fixed_bits_keys_env() -> int:
    raw = os.getenv("BSI_FIXED_BITS_KEYS", os.getenv("BSI_FIXED_BITS", "0"))
    try:
        value = int(raw)
    except Exception:
        value = 0
...
def _estimate_bsi_layer_base_bytes(module: torch.nn.Linear) -> int:
    ...
    if slices <= 0:
        return int(module.out_features * module.in_features * 4)
    words_per_slice = (module.in_features + 63) // 64
    words_bytes = int(module.out_features * slices * words_per_slice * 8)
    weights_bytes = int(module.out_features * slices * 4)
