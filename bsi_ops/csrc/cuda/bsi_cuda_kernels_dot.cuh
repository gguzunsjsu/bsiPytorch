...
void popcount_weighted_keys_literal_fused_bmma_tc_kernel_tm16_fixed76_chunkscale(
    ...
    float* __restrict__ out_global)
{
    ...
    constexpr size_t A_words = (size_t)SA * (size_t)TM * (size_t)K_STRIDE32;
...

    {
        ...
        for (int idx = threadIdx.x; idx < TM * SA * K_WORDS64_16B; idx += blockDim.x) {
            int t = idx;
    ...
    for (int chunk = 0; chunk < chunks; ++chunk) {
        ...
        float qscale_m1 = 1.0f;
        if (threadID == 0) {
            const int q_m0 = q0 + row0;
            const int q_m1 = q0 + row1;
            qscale_m0 = __ldg(&A_chunk_scales[(size_t)q_m0 * (size_t)A_scale_stride + (size_t)chunk]);
            qscale_m1 = __ldg(&A_chunk_scales[(size_t)q_m1 * (size_t)A_scale_stride + (size_t)chunk]);
        }
        qscale_m0 = __shfl_sync(0xffffffff, qscale_m0, lane & ~3);
...
void popcount_weighted_keys_literal_fused_bmma_tc_kernel_tm32_fixed76_chunkscale_lr(
    const unsigned long long* __restrict__ A, // [Q, 7, W64]
    const float* __restrict__ A_chunk_scales, // [Q, chunks]
    int A_scale_stride,
    int W64,
    ...
    float* __restrict__ out_global)
{
    ...
    p += A_words * sizeof(uint32_t);
    auto* B_bits = reinterpret_cast<uint32_t*>(p);
    p += B_words * sizeof(uint32_t);
    (void)p;
    ...
    for (int chunk = 0; chunk < chunks; ++chunk) {
        for (int idx = threadIdx.x; idx < TM_TOTAL * SA * K_WORDS64_16B; idx += blockDim.x) {
            ...
            bsi_cp_async_cg_16B(
                A_bits + base,
                &a_slice[(size_t)chunk * (size_t)K_WORDS64 + (size_t)w64_i]);
        }
        for (int idx = threadIdx.x; idx < TN * SB * K_WORDS64_16B; idx += blockDim.x) {
            int t = idx;
            const int w64_pair = t & (K_WORDS64_16B - 1);
            ...
            bsi_cp_async_cg_16B(
                B_bits + base,
                &b_slice[(size_t)chunk * (size_t)K_WORDS64 + (size_t)w64_i]);
        }
        bsi_cp_async_commit_group();
        bsi_cp_async_wait_all();
        __syncthreads();
    ...
    if (use_tc) {
        ...
        if (cached_tc_ok && Sa > 0 && Sb > 0 && (W % 4 == 0) && W >= 4) {
            ...
            if (cached_fixed_int < 0) {
                ...
                cached_fixed_int = (v < 0) ? 0 : v;
            }
            const int fixed_int_mode = cached_fixed_int;
            ...
            const bool full_tiles_tm16 = ((Q & 15) == 0) && ((R & 31) == 0);
            ...
            auto maybe_debug_print_fixed_int = [&](int tm, int mode, size_t smem) {
                if (debug <= 0) return;
                if (debug == 1 && debug_printed) return;
                if (debug == 1) debug_printed = 1;
                const int chunks = W >> 2;
                const long long work = (long long)chunks * 7ll * 6ll;
                fprintf(
                    stderr,
                    "[BSI_DOT] tc_fixed_int=1 mode=%d tm=%d cpasync=1 Sa=7 Sb=6 Q=%d R=%d W64=%d chunks=%d work=%lld smem=%zu\n",
                    mode,
                    tm,
                    Q,
                    R,
            ...
            auto try_tm32 = [&]() -> bool {
                ...
                if (fixed_int_mode > 0 && use_cpasync_requested && fixed76 && identity && full_tiles_tm32 && chunk_scale) {
                    ...
                    if (fixed_int_mode >= 2) {
                        ...
                        if (shared_bytes <= (size_t)max_shared) {
                            popcount_weighted_keys_literal_fused_bmma_tc_kernel_tm32_fixed76_chunkscale_lr<<<
                                ...
                                R_total,
                                out_global);
                            maybe_debug_print_fixed_int(32, fixed_int_mode, shared_bytes);
                            return true;
                        }
                    } else {
                        constexpr int stages = 2;
            ...
            auto try_tm16 = [&]() -> bool {
                ...
                if (fixed_int_mode > 0 && use_cpasync_requested && fixed76 && identity && full_tiles_tm16 && chunk_scale) {
                    constexpr int K_WORDS32 = 8;
                    constexpr int K_STRIDE32 = K_WORDS32 + 4;
                    constexpr int stages = 2;
                    size_t shared_bytes =
                        16u +
                        (size_t)stages * (size_t)7 * (size_t)TM * (size_t)K_STRIDE32 * sizeof(uint32_t) +
                        (size_t)stages * (size_t)6 * (size_t)TN * (size_t)K_STRIDE32 * sizeof(uint32_t);
                    if (shared_bytes <= (size_t)max_shared) {
                        dim3 grid_fixed(R / TN, Q / TM, 1);
                        popcount_weighted_keys_literal_fused_bmma_tc_kernel_tm16_fixed76_chunkscale<<<
                            ...
                            R_total,
                            out_global);
                        maybe_debug_print_fixed_int(16, fixed_int_mode, shared_bytes);
                        return true;
                    }
                }
