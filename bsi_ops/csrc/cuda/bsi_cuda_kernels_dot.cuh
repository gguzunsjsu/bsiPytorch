            ...
            auto try_tm32 = [&]() -> bool {
                ...
                dim3 grid_tc((R + TN - 1) / TN, (Q + TM_TOTAL - 1) / TM_TOTAL, 1);
	                if (cached_fixed_int && use_cpasync_requested && fixed76 && identity && full_tiles_tm32 && chunk_scale) {
                        ...
                        // Large-shape policy: prioritize occupancy and lower shared-memory pressure.
                        // OPT-1.3B-like shapes (W=32, R=2048) should deterministically select rsweep=2.
                        const bool large_shape = (W >= 24) || (R >= 1536) || ((long long)Q * (long long)R >= (1ll << 17));
                        const bool medium_shape = (W >= 20) || (R >= 1024) || ((long long)Q * (long long)R >= (1ll << 16));
                        if (r_sweep == 0) {
                            r_sweep = large_shape ? 2 : 4;
                            // Prefer rsweep=2 whenever the shape is not a clean rsweep4 fit or is at least medium.
                            if ((R % (TN * r_sweep)) != 0) {
                                r_sweep = ((R % (TN * 4)) == 0 && !medium_shape) ? 4 : 2;
                            }
                        }

                        // Hard safety valve for experiments and for auto-large shapes.
                        ...
                        if (cached_force_rs2 || large_shape) {
                            r_sweep = 2;
                        }
                        ...
                        if (cached_tc_tma == 1) {
                            ...
                            // Auto mode: disable TMA for the common OPT-125M/1.3B tuning cases.
                            // Only consider TMA on much larger shapes where rsweep=2 is already active.
                            want_tma = (r_sweep == 2) && (W >= 64) && (R >= 8192) && (((long long)Q * (long long)R) >= (1ll << 20));
                        }
                        // Safety valve: suppress TMA by default for medium/large fixed76 shapes.
                        ...
                        if (cached_disable_tma_large && (large_shape || medium_shape || r_sweep == 2)) {
                            want_tma = false;
                        }
                        if (want_tma) {
	                    ...
	                    if (r_sweep > 1 && r_main >= tn_sweep) {
                            size_t shared_bytes = fixed76_shared_bytes(r_sweep, use_tma != 0);
                            // Shared-memory guardrail: if the selected path is too heavy, fall back to rsweep=2 without TMA.
                            if ((r_sweep == 4 && shared_bytes > (size_t)max_shared_default) ||
                                shared_bytes > (size_t)max_shared) {
                                r_sweep = 2;
                                use_tma = 0;
                                B_tensor_map = nullptr;
                                shared_bytes = fixed76_shared_bytes(r_sweep, false);
                            }
