    ...
    cudaStream_t stream)
{
    ...
    if (use_tc) {
        ...
        if (cached_tc_ok && Sa > 0 && Sb > 0 && (W % 4 == 0) && W >= 4) {
            ...
            auto maybe_debug_print_fixed_int = [&](int tm) {
                ...
                fprintf(
                    stderr,
                    "[BSI_DOT] tc_fixed_int=1 tm=%d cpasync=1 Sa=7 Sb=6 W64=%d chunks=%d work=%lld\n",
                    tm,
                    W,
                    chunks,
                    work);
                ...
                if (cached_fixed_int && use_cpasync_requested && fixed76 && identity && full_tiles_tm32 && chunk_scale) {
                    ...
                    if (shared_bytes <= (size_t)max_shared) {
                        popcount_weighted_keys_literal_fused_bmma_tc_kernel_tm32_fixed76_chunkscale<<<
                            ...
                            R_total,
                            out_global);
                        maybe_debug_print_fixed_int(32);
                        return true;
                    }
                }
            ...
            auto try_tm16 = [&]() -> bool {
                ...
                if (cached_fixed_int && use_cpasync_requested && fixed76 && identity && full_tiles_tm16 && chunk_scale) {
                    constexpr int K_WORDS32 = 8;
                    constexpr int K_STRIDE32 = K_WORDS32 + 4;
                    constexpr int stages = 2;
                    size_t shared_bytes =
                    ...
                    if (shared_bytes <= (size_t)max_shared) {
                        ...
                        popcount_weighted_keys_literal_fused_bmma_tc_kernel_tm16_fixed76_chunkscale<<<
                            ...
                            R_total,
                            out_global);
                        maybe_debug_print_fixed_int(16);
                        return true;
                    }
                }
