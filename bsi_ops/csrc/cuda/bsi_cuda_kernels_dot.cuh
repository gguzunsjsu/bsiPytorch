...
extern "C" __global__
void popcount_weighted_keys_literal_fused_bmma_tc_kernel_packed(
    ...
    int Sa,
    int W64,
    const uint32_t* __restrict__ B_tc,           // [R, Sb, chunks, 12]
    const float* __restrict__ Bw,                // [R, Sb]
    int Sb,
    int R,
    int Q,
    ...
    float* __restrict__ out_global)
{
    ...
    const int a_row_stride = Sa * chunks * K_STRIDE32;
    const int a_slice_stride = chunks * K_STRIDE32;
    const int b_row_stride = Sb * chunks * K_STRIDE32;
    const int b_slice_stride = chunks * K_STRIDE32;

    for (int chunk = 0; chunk < chunks; ++chunk) {
        for (int idx = threadIdx.x; idx < TM * Sa * K_WORDS32; idx += blockDim.x) {
            int t = idx;
            const int k32 = t & (K_WORDS32 - 1);
            t >>= 3;
            const int m = t & (TM - 1);
            const int i = t >> 4;
    ...
    int use_cpasync)
{
    ...
    if (can_cpasync) {
        uint32_t* A_bits = A_bits0;
        uint32_t* B_bits = B_bits0;
        constexpr int K_GROUPS_16B = K_WORDS32 / 4; // 2
        for (int idx = threadIdx.x; idx < TM_TOTAL * Sa * K_GROUPS_16B; idx += blockDim.x) {
            int t = idx;
            const int g = t & (K_GROUPS_16B - 1);
            t >>= 1;
            const int m = t & (TM_TOTAL - 1);
            const int i = t >> 5;
            const int q = q0 + m;
    ...
    for (int chunk = 0; chunk < chunks; ++chunk) {
        ...
        if (!can_cpasync) {
            ...
            if (chunk + 1 < chunks) {
                ...
                uint32_t* B_bits_next = (next_stage == 0) ? B_bits0 : (B_bits0 + B_words);
                const int next_chunk = chunk + 1;
                constexpr int K_GROUPS_16B = K_WORDS32 / 4;
                for (int idx = threadIdx.x; idx < TM_TOTAL * Sa * K_GROUPS_16B; idx += blockDim.x) {
                    int t = idx;
                    const int g = t & (K_GROUPS_16B - 1);
                    t >>= 1;
    ...
    cudaStream_t stream)
{
    ...
    if (!cached_tc_ok) return;
    if (Sa <= 0 || Sb <= 0 || Q <= 0 || R <= 0) return;
    if ((W64 & 3) != 0 || W64 < 4) return;
    ...
    auto maybe_debug_print = [&](int tm, int cpasync, size_t shared_bytes) {
        if (!debug || debug_printed) return;
        debug_printed = 1;
        const int chunks = W64 >> 2;
        const long long work = (long long)chunks * (long long)Sa * (long long)Sb;
        fprintf(
            stderr,
            "[BSI_DOT] tc_packed=1 tm=%d cpasync=%d Sa=%d Sb=%d W64=%d chunks=%d work=%lld smem=%zu\n",
            tm,
            cpasync,
            Sa,
            Sb,
    ...
    auto try_tm32 = [&]() -> bool {
        ...
        if (shared_bytes <= (size_t)max_shared) {
            ...
            popcount_weighted_keys_literal_fused_bmma_tc_kernel_tm32_packed<<<grid_tc, block_tc, shared_bytes, stream>>>(
                A_tc,
                Aw,
                A_chunk_scales,
                A_scale_stride,
                ...
                out_global,
                use_cpasync);
            maybe_debug_print(32, use_cpasync, shared_bytes);
            return true;
        }
        return false;
    };
    ...
    auto try_tm16 = [&]() -> bool {
        ...
        if (shared_bytes <= (size_t)max_shared) {
            ...
            popcount_weighted_keys_literal_fused_bmma_tc_kernel_packed<<<grid_tc, block_tc, shared_bytes, stream>>>(
                A_tc,
                Aw,
                A_chunk_scales,
                A_scale_stride,
                ...
                R_total,
                out_global);
            maybe_debug_print(16, 0, shared_bytes);
            return true;
        }
        return false;
    };
